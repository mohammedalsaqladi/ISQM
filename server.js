...
app.post('/api/auth/login', wrap(async (req, res) => {
  ...
  const r = await query(
    `SELECT u.id, u.firm_id, u.username, u.password_hash, u.is_active, u.full_name_ar, u.full_name_en
  ...
  const token = jwt.sign({ uid: u.id, fid: u.firm_id, username: u.username }, SECRET, { expiresIn: '12h' });
...
app.post('/api/auth/register', wrap(async (req, res) => {
  const { firmName, cr, city, adminName, email, username, password } = req.body || {};
  ...
  const token = jwt.sign({ uid: out.uid, fid: out.firmId, username }, SECRET, { expiresIn: '12h' });
...
app.get('/api/users', auth, wrap(async (req, res) => {
  ...
  res.json(r.rows.map((u) => ({
    ...
    active: u.is_active, last: u.last_login_at ? `${dpart(u.last_login_at)} ${tpart(u.last_login_at)}` : '—',
...
app.post('/api/responses', auth, wrap(async (req, res) => {
  ...
  const out = await tx(async (c) => {
    const n = await c.query(
      `SELECT COUNT(*)::int AS n FROM responses WHERE firm_id=$1`, [req.user.fid]);
    const code = 'RS-' + String(n.rows[0].n + 1).padStart(2, '0');
