...
app.post('/api/auth/register', wrap(async (req, res) => {
  ...
  if (idemKey) {
    const existing = await query(
      'SELECT company_id, user_id, code FROM registration_attempts WHERE idempotency_key=$1',
      [idemKey]);
    if (existing.rowCount && existing.rows[0].company_id) {
      ...
      return res.json({ token, firmCode: row.code, reused: true });
    }
  }
  ...
  try {
    out = await tx(async (c) => {
      ...
      if (idemKey) {
        await c.query(
          `UPDATE registration_attempts SET company_id=$1, user_id=$2, code=$3 WHERE idempotency_key=$4`,
    ...
    if (e.code === 'DUP_SUBMIT') {
      const prev = await query(
        'SELECT company_id, user_id, code FROM registration_attempts WHERE idempotency_key=$1', [idemKey]);
